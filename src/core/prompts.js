 ...
 * 规范）+ 情境层（年级、考区、用户自定义追加）。App 侧由 `scripts/sync-from-plugin.mjs`
 * 把本文件原样复制成 `highschool-tutor-app/src/core/prompts.js`（两边逐字节相同），
 * 所以这里**不要 import 任何其它模块**（保持零依赖，两个仓库的路径结构不同）。
 *
 * 两边用法不同，故有两套组装：
